...
- 全局安装：当前主机 `~/.agents/skills/ai-capability-workflow/` 已安装；`~/.codex/AGENTS.md` 的原有四条规则保留，并新增仅针对 Skill / Agent 开发的触发条件。源文件与全局副本的 `SKILL.md` SHA-256 一致。
...
| 分享压缩包 | 初版压缩包含 `SKILL.md` 和任务清单模板，共 11 个条目；发布版补入许可证，共 12 个条目 |
...
| T05 同一样本的跨环境行为验证 | 部分完成 | 发布前 Codex CLI 已通过正反触发样本；其他 harness 本机不可用，尚未实测 |
